...
export const DeleteCustomAppPromptPage: FC = () => {
        ...
        deleteCustomApp()
            .then(() =>
                notify(
                    ...
                    translate('akeneo_connectivity.connection.connect.marketplace.custom_apps.delete.flash.success')
                )
            )
            .catch(() =>
                notify(
                    NotificationLevel.ERROR,
                    translate('akeneo_connectivity.connection.connect.marketplace.custom_apps.delete.flash.error')
                )
            )
            .finally(() => history.push(generateUrl('akeneo_connectivity_connection_connect_marketplace')));
    }, [deleteCustomApp, notify, translate, history, generateUrl]);
    ...
    return (
        <Modal onClose={handleCancel} illustration={<AppIllustration />} closeTitle={translate('pim_common.cancel')}>
            <Subtitle>
                {translate('akeneo_connectivity.connection.connect.marketplace.custom_apps.delete.subtitle')}
            </Subtitle>
            <Title>{translate('akeneo_connectivity.connection.connect.marketplace.custom_apps.delete.title')}</Title>
            <Helper>
                <p>{translate('akeneo_connectivity.connection.connect.marketplace.custom_apps.delete.description')}</p>
                <p>{translate('akeneo_connectivity.connection.connect.marketplace.custom_apps.delete.warning')}</p>
            </Helper>
            <Modal.BottomButtons>
